...
/*Custom view for Satellite plot*/
public class Skyview extends ConstraintLayout {
    Random rand = new Random();
    private AzimElev mLatLongCenter = null;
    private Context mContext;
    private double mRadarDistance;
    private ArrayList<Skymodel> mDataSets;
    int mWidthHeight = 0;
    public View mCenterView;
    Skyview mLayout;
    int rand_int1 = rand.nextInt(1000);
    int idCenterLayout = rand_int1;
    private Skyview.IRadarCallBack radarCallBack;


    public void setupData(double mRadarDistance, ArrayList<Skymodel> mDataSets, AzimElev mLatLongCenter, View centerView) {
        try{
    ...
    private void startDrawRadar() {
        try {
            if (mContext == null || mRadarDistance <= 0 || mDataSets == null) {
                return;
            }
    ...
    private void setupDataOnRadar() {
        try{
            ...
            for (int position = 0; position < mDataSets.size(); position++) {
                ...
                constraintSet.constrainCircle(viewD.getId(), mCenterView.getId(), mDataSets.get(position).getmElev(), mDataSets.get(position).getmAzim());
    ...
    public void setUpCallBack(Skyview.IRadarCallBack radarCallBack) {
